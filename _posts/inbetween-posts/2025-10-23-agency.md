...
**Attempt 2: Agency as Controllability**

> tldr: measure the set of reachable futures

A more sophisticated model comes from control theory and reinforcement learning: the concept of **controllability**. We could define an individual's agency as the size of the set of all possible future states they can guarantee they can reach. For a commuter, this would be the set of all intersections on the grid they can choose to arrive at.

This is a powerful concept because it's active and forward-looking; it measures an agent's power to *drive* the future to a desired state. However, it has a critical flaw when applied to a multi-agent world: **it implicitly assumes other agents are passive parts of the environment.** A commuter might be able to reach any intersection on an empty map, but they cannot guarantee they can reach the next intersection in one minute if another strategic commuter can decide to block it. The actions of others shrink your set of truly controllable states, making this definition inadequate.

**Attempt 3: Agency as Probabilistic Futures**

> tldr: weight possible futures by their likelihood

To account for the actions of others, we could soften the definition and weight each future state by its probability. This is a significant improvement, but it can feel passive. It measures the futures that are likely to happen *to* a commuter, given the typical flow of traffic. It doesn't fully capture their power to strategically adapt to and influence the system.

**Attempt 4: Agency as the Set of Achievable Goals**

> tldr: measure the number of goals achievable

Let's try one final, more powerful definition for an individual's agency: **the size of the set of goals they can achieve.** This definition is attractive because it's active, goal-oriented, and—crucially—it remains neutral about the nature of the goals themselves.

However, this definition has a fatal flaw: **an individual's set of achievable goals is not a stable, intrinsic property of that individual.** Consider a simple goal: "park in the last open spot at the destination." Your ability to achieve this goal depends entirely on the goals and actions of others. If another commuter also has that goal and their route is shorter, the goal is likely *not* in your set.

The set of goals you can achieve is radically contingent on the goals and strategies of everyone around you. It is not a property of *you*; it is an emergent property of the *system*. This is the core insight. Because an individual's agency is so fundamentally entangled with the state of the entire system, any attempt to calculate it as a single, isolated number is a futile exercise. The object of measurement is wrong.
