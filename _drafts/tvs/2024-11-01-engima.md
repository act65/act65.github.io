...
This cipher can be mathematically described using modular arithmetic. If we map letters a-z to numbers 0-25, the encryption of a letter $x$ with a shift $k$ is $E(x) = (x + k) \pmod{26}$.
...
<div class="interactive-section">
    ...
    <input type="text" id="inputTextCesar" placeholder="hello world" value="hello world" class="large-input">
    <br>
...
A substitution cipher is a method of encrypting in which units of plaintext are replaced with ciphertext, according to a fixed system. In a simple monoalphabetic substitution cipher, each letter of the alphabet is mapped to a unique different letter. The "key" is this permutation of the alphabet.
...
<div class="interactive-section">
    ...
    <input type="text" id="inputTextSubstitution" placeholder="secret message" value="secret message" class="large-input">
    <br>
...
<div class="interactive-section">
    ...
    <input type="text" id="inputTextStepping" placeholder="attack at dawn" value="attack" class="large-input">
    <br>
...
2.  **Two Rotors (`rotor_wirings`, `rotor_inv_wirings`):**
    The machine features two rotors. The wiring for each rotor is a permutation of the `n` possible inputs. This means each number from `0` to `n-1` is mapped to a unique number within the same range.
    *   In the `EasyEnigma` struct, `rotor_wirings: [Vec<u32>; 2]` stores these two permutations.
    *   These permutations are initially created by the `generate_permutation(n, rng)` function when a new `EasyEnigma` instance is made.
    *   For the signal to travel backward through the rotors during encryption, the machine also stores their inverse permutations: `rotor_inv_wirings: [Vec<u32>; 2]`. These are calculated by the `invert_permutation(perm: &[u32])` function.
...
4.  **Stepping Mechanism (`step`):**
    ...
    *   `EasyEnigma` uses two counters, `step: [u32; 2]`, stored in the struct, representing the current rotational offset of each of the two rotors. These are typically initialized to ``.
    *   The stepping logic is found at the end of the `call_char` method:
        ```rust
        // Step for the next character
        self.step[0] = (self.step[0] + 1) % self.n;
        if self.step[0] == 0 { // If the first rotor completes a full revolution
            self.step[1] = (self.step[1] + 1) % self.n; // Then the second rotor steps
        }
        ```
...
5.  **Encryption Path (within `call_char` method):**
    ...
    *   **Forward Pass through Rotors:**
        The input `x` first passes through `rotor` and then `rotor`. The transformation through each rotor is handled by the `pass_rotor` helper function.
        ```rust
        // Forward path
        ...
        ```
        The `pass_rotor` function (`fn pass_rotor(val: u32, rotor_idx: usize, step_val: u32, forward: bool, n: u32, wirings: &[Vec<u32>], inv_wirings: &[Vec<u32>]) -> u32`) works like this:
        ...
        2.  `wired_output = wirings[rotor_idx][effective_input as usize];` (if `forward` is true): The shifted value then passes through the rotor's actual fixed wiring.
    ...
    *   **Reflector:**
        The output from the second rotor then enters the `reflector_wiring`.
        ```rust
        // Reflector
    ...
    *   **Backward Pass through Rotors:**
        The reflected value then travels back through the rotors, but in reverse order (`rotor` then `rotor`) and using their inverse wirings. The `pass_rotor` function is used again, but with `forward` set to `false`, which makes it use the `rotor_inv_wirings`.
        ```rust
        // Backward path
...
6.  **Initialization and Configuration:**
    An `EasyEnigma` instance is created using the `EasyEnigma::new(n: u32, rng: &mut impl Rng) -> Self` constructor, which sets up the random rotor wirings and reflector for a given alphabet size `n`.
    Users can also define specific configurations using `set_wirings(...)` or adjust the initial rotor positions using `set_steps(...)`. The `reset_steps()` method returns the rotors to their `` position.
...
Our `EasyEnigma` aims to demonstrate the core ideas of a rotor cipher, but it's important to understand its simplifications compared to the historical Enigma machines used in WWII:
...
*   **Key Differences:**
    *   **Complexity:** `EasyEnigma` has 2 rotors and a simple odometer-style stepping. Historical Enigmas had 3-4 rotors (chosen from a larger set) with more complex, notched stepping mechanisms.
    ...
    *   **Cryptographic Robustness:** Due to these simplifications, `EasyEnigma` is an educational tool and far less secure than the historical Enigma.
...
We've journeyed from simple ciphers like Caesar's to a more complex (though still simplified) `EasyEnigma` rotor system. You've seen how polyalphabetic ciphers improve upon monoalphabetic ones and got a glimpse into how one might begin to analyze such ciphers by understanding their internal mechanics. Real-world cryptography is vastly more sophisticated, employing advanced mathematics and computational theory to secure information. However, understanding these foundational concepts is the first step into this fascinating field!
