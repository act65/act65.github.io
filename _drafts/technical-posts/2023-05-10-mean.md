...
Data can often be noisy, messy, and hard to interpret. We need tools to summarize it, and a **mean** is one such tool. It seeks to describe the 'center' or 'typical value' of the data. Of course, the 'center' is just one aspect. Other summaries describe how data is spread out (like variance and standard deviation) or the shape of its distribution (skewness and kurtosis).

But what exactly *is* a mean? And why are there so many? The "right" definition depends on the nature of your data and what you want to achieve with your summary.

#### The Arithmetic Mean (AM)

The arithmetic mean is likely the first type of mean you encountered. It's calculated by summing all the values and dividing by the count of values:
...
*   **Generalizing GM/HM for non-positive numbers:**
    *   **GM:** If any value is 0, GM is 0. With negative numbers, if there's an odd count of them and $n$ is even, the GM involves taking an even root of a negative number, leading to complex numbers (or being undefined in reals). It's most coherently used for positive values.
    *   **HM:** Can be calculated with negative numbers, but it's undefined if any value is 0 or if the sum of reciprocals is 0 (e.g., $H(1, -1)$ is undefined).
...
## 'Typical' Samples and the Mean <a name="typical"></a>
